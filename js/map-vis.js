...
MapVis.prototype.initVis = function () {
  ...
  // Column 2
  vis.xCol2 = 650;
  vis.barY = d3.scaleBand().range([vis.height, 0]);

  vis.barX = d3.scaleLinear().range([0, vis.width]);

  vis.svgCol2 = vis.svg
      .append('g')
      .attr('class', 'map-col2')
      .attr('transform', 'translate(' + vis.xCol2 + ',-120)');
  ...
  vis.mapGroup = vis.svg.append('g').attr('class', 'map');
...
MapVis.prototype.wrangleData = function () {
  ...
  vis.mapData = topojson.feature(
      vis.mapDataRaw,
      vis.mapDataRaw.objects.countries
  ).features;
...
MapVis.prototype.updateVis = function () {
  ...
  vis.color.domain([0, 10, 50, 100, 500, 750, 1000]);
  ...
  var chmap = vis.mapGroup.selectAll('.mapPath').data(vis.countryInfo);
  ...
  chmap
      ...
      .merge(chmap)
      .on('mouseover', (d, data) => {
        let count = !!vis.idToRevenue[data.id] ? vis.idToRevenue[data.id] : 0;
        vis.tooltip.transition().duration(800).style('opacity', 0.8);
        var txt = vis.idToCountry[data.id] + '<br>' + count;
        vis.tooltip
            .html(txt)
      ...
      .on('mouseout', (d) => {
        vis.tooltip.transition().duration(600).style('opacity', 0);
      })
      .attr('fill', function (d, i) {
        if (d.id in vis.idToRevenue) return vis.color(vis.idToRevenue[d.id]);
        return vis.color(0);
      });
  chmap.exit().remove();
  ...
  var yLegend = d3
      ...
      .domain([1, length - 1])
      .rangeRound([
        (legendHeight * (length - 1)) / length,
        legendHeight / length,
      ]);

  var legendRects = vis.svg.selectAll('.legend-rect').data(vis.color.range());
  ...
  var texts = vis.svg.selectAll('.texts').data(legendTexts);
  ...
  var sharkTitle = vis.svgCol2
      .selectAll('.shark-name')
  ...
  var u = vis.pieGroup.selectAll('.pie').data(data_ready);
  ...
  var legendPie = vis.svgCol2.selectAll('.pie-legend').data(pieColors);
  ...
  var legendPieText = vis.svgCol2
      .selectAll('.pie-legend-text')
      .data(pieLegends);
...
MapVis.prototype.updateVis = function () {
  ...
  vis.color.domain([0, 10, 50, 100, 500, 700, 1000]);
